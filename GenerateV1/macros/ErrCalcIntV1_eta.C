...
TH1D * v1oddStat_eta[nanals][ncentbins];
TH1D * v1evenStat_eta[nanals][ncentbins];

TH1D * v1oddSyst_eta[nanals][ncentbins];
TH1D * v1evenSyst_eta[nanals][ncentbins];

TH1D * PerErrv1odd_eta[nanals][ncentbins];
TH1D * PerErrv1even_eta[nanals][ncentbins];

TH1D * runParms[nanals];

void ErrCalcIntV1_eta()
{
    ...
    for (int i = 0; i<nanals; i++) {
        for (int cbin = 0; cbin<ncentbins; cbin++) {
            ...
            PerErrv1odd_eta[i][cbin] = new TH1D(Form("PerErrv1odd_eta_%s_%d",AnalNames[i].data(),cbin), "", netabins, etabins);
            PerErrv1even_eta[i][cbin] = new TH1D(Form("PerErrv1even_eta_%s_%d",AnalNames[i].data(),cbin), "", netabins, etabins);

            v1oddStat_eta[i][cbin] = new TH1D(Form("v1oddStat_eta_%s_%d",AnalNames[i].data(),cbin), "", netabins, etabins);
            v1evenStat_eta[i][cbin] = new TH1D(Form("v1evenStat_eta_%s_%d",AnalNames[i].data(),cbin), "", netabins, etabins);

            v1oddSyst_eta[i][cbin] = new TH1D(Form("v1oddSyst_eta_%s_%d",AnalNames[i].data(),cbin), "", netabins, etabins);
            v1evenSyst_eta[i][cbin] = new TH1D(Form("v1evenSyst_eta_%s_%d",AnalNames[i].data(),cbin), "", netabins, etabins);
        }
    }

    // symmetrize v1^odd and v1^even
    for (int i = 0; i<nanals; i++) {
        for (int cbin = 0; cbin<ncentbins; cbin++) {
            for (int j = 0; j<netabins; j++) {
                double v1odd_pos = v1odd_eta[i][cbin]->GetBinContent(netabins-j);
                double v1odd_neg = v1odd_eta[i][cbin]->GetBinContent(j+1);
                double v1odd_pos_err = v1odd_eta[i][cbin]->GetBinError(netabins-j);
                double v1odd_neg_err = v1odd_eta[i][cbin]->GetBinError(j+1);

                double v1odd = 0.5*(v1odd_pos + v1odd_neg);
                // if (j<6 || (v1odd_pos < 0 && v1odd_neg < 0)) v1odd*=-1;
                double v1odd_stat_err = 0.5*sqrt( v1odd_pos_err*v1odd_pos_err + v1odd_neg_err*v1odd_neg_err );
                //if (cbin == 0 && i == 7) cout<<v1odd<<"\t"<<v1odd_stat_err<<endl;
                // v1oddStat_eta[i][cbin]->SetBinContent(netabins-j, v1odd);
                // v1oddStat_eta[i][cbin]->SetBinError(netabins-j, v1odd_stat_err);
            }
        }
    }
